...
function RestaurantRow({ restaurant, isFavorite, onToggleFavorite, onUpdateRestaurant }) {
	  const r = restaurant;
		        ...
		        <tr>
		          ...
		          <td>
		            <button
		              ...
		              title={activeMoods.length ? activeMoods.join(", ") : "Set mood"}
		            >
		              {activeMoods.length
				                    ? activeMoods.map((m) => MOOD_EMOJI[m] || m).join(" ")
				                    : "+ Mood"}
		            </button>
		          </td>
		          <td>
				          ...
				          <td colSpan={6} style={{ padding: 0, border: "none" }}>
				            <div className="modal-backdrop" onClick={() => setShowMoodModal(false)}>
				              <div className="modal" onClick={(e) => e.stopPropagation()}>
				                <div className="modal-header">
				                  <h3>Mood — {r.name}</h3>
				                  <button className="modal-close" onClick={() => setShowMoodModal(false)}>×</button>
				                </div>
				                ...
				                <div className="form-actions">
				                  <button className="btn-primary" onClick={() => setShowMoodModal(false)}>Done</button>
				                </div>
				              </div>
				            </div>
				          </td>
