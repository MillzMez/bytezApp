    ...
    <div className="modal-backdrop" onClick={handleBackdropClick}>
      <div className="modal random-modal">
        ...
        <div className="random-pick-card">
          <div className="random-pick-name">{restaurant.name}</div>
          <div className="random-pick-meta">
            {restaurant.cuisine} · {"$".repeat(restaurant.price)} ·{" "}
            <span className="star">★</span> {restaurant.rating.toFixed(1)}
          </div>
          <div className="random-pick-address">📍 {restaurant.address}</div>
          {restaurant.notes && (
            <div className="random-pick-notes">📝 {restaurant.notes}</div>
          )}
        </div>
